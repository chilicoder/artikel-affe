...
;define("artikel-affe/controllers/der-die-das", ["exports"], function (_exports) {
  ...
  var _default = Ember.Controller.extend({
    setNextIndex() {
      ...
      if (last.result) {
        if (stashOfFails[count]) {
          if (stashOfFails[count].rightAnswers <= 4) {
            offset = stashOfFails[count].rightAnswers * 5 + 8 + Math.floor(Math.random() * 5); // if step is occupied then rewind to free space
      ...
      delete stashOfFails[count]; // setting index for next word 
    ...
    stashOfFails: Ember.computed(function () {
      return {};
    }),
...
;
          if (!runningTests) {
            require("artikel-affe/app")["default"].create({"name":"artikel-affe","version":"0.0.0+dec5b907"});
          }
